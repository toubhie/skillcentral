...
class Crud_model extends CI_Model
{
    ...
    public function add_lesson()
    {
        ...
        public function edit_lesson($lesson_id)
        {
            ...
            public function enrol_to_free_course($course_id = "", $user_id = "")
            {
                ...
                if ($course_details['is_free_course'] == 1) {
                    ...
                    if ($this->db->get_where('enrol', $data)->num_rows() > 0) {
                        ...
                        $amount_paid = "Free";
                        $this->crud_model->free_course_purchase($user_id, 'Free', $amount_paid, $course_id);
                        $this->email_model->free_course_purchase_notification($user_id, 'Free', $amount_paid, $course_id);
            ...
            public function free_course_purchase($user_id, $method, $amount_paid, $course_id)
            {
                $data['user_id'] = $user_id;
                ...
                function get_application_details()
                {
                    ...
                    function enrol_to_free_course_mobile($course_id = "", $user_id = ""){
                        ...
                        if($this->db->get_where('course', array('id' => $course_id))->row('is_free_course') == 1):
                            ...
                            $amount_paid = "Free";
                            $this->crud_model->free_course_purchase($user_id, 'Free', $amount_paid);
                            $this->email_model->course_purchase_notification($user_id, 'Free', $amount_paid);
                        endif;
                    }
