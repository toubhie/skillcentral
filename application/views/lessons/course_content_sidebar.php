                    ...
                    <div class="tab-pane fade" id="liveclass" role="tabpanel" aria-labelledby="liveclass-tab" style="text-align: center;">
                        ...
                        if ($live_class->num_rows() > 0):
                            ...
                            <div style="padding: 30px 0px;">
                                ...
                                <!--
                                <a href="<?php //echo site_url('addons/liveclass/join/'.$course_id);?>" class="btn btn_zoom" target="_blank">
                                ...
                                <p><b>NOTE:</b> If you have any issues joining the webinar using the button above, use the alternate link below:</p>
                                <br>
                                <a href="https://us02web.zoom.us/j/88432802897?pwd=bUJmQlJhbUkzTlFYb3kxM1NRcTV1UT09" class="btn btn_zoom" target="_blank">
                                ...
                                <p><b>MICROSOFT TEAMS :</b> </p>
                                <br> -->
                                
                                
                                <a href="https://teams.microsoft.com/l/meetup-join/19%3ameeting_ZjA5YmFhZWItZDk4Ni00NmU1LWJhMDEtZDY5MTM3YmUyODNm%40thread.v2/0?context=%7b%22Tid%22%3a%22e0f78b9c-7b47-42bf-af17-cfc70325ec96%22%2c%22Oid%22%3a%22f35193ba-88c2-484c-89be-80696a5ae095%22%2c%22IsBroadcastMeeting%22%3atrue%7d" 
                                    class="btn btn_zoom" target="_blank">
                                    <i class="fa fa-link"></i>&nbsp;
                                    Join via Microsoft Teams
                                </a>
                            </div>
                        <?php else: $is_live_class = "false" ?>
                    ...
                    <div id="download_certificate_area" style="padding: 15px;">
                        <?php if (addon_status('certificate')): ?>
                            <span id = "total_price_of_checking_out" hidden>
                                <?php $total_price = 1000; 
                                $this->session->set_userdata('total_price_of_checking_out', $total_price);
                                $this->session->set_userdata('course_id', $course_id);
                                $this->session->set_userdata('is_certificate_payment', 1); ?>
                            </span>

                            <a href="#" onclick="handleCertificatePayment()" class="btn btn-primary" id = "certificate_download_btn" disabled><?php echo get_phrase('get_certificate'); ?></a>
                        <?php else: ?>
...
function handleCertificatePayment() {
    var result = confirm("Do you want to pay N1,000 for a Certificate?");
        ...
        $.ajax({
            ...
            success: function(response)
            {
                if (!response) {
                    ...
                    toastr.error('<?php echo get_phrase('there_are_no_courses_on_your_cart');?>');
                }
            }
        });
    } else{
